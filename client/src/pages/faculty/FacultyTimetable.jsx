...
const FacultyTimetable = () => {
    ...
    const renderCell = (day, p) => {
        ...
        if (!entry) {
            return (
                <td key={p.id} className="p-3 border border-gray-100 bg-gray-50/30 text-center text-gray-400 text-xs">
                    -
                </td>
            );
        }
        ...
        return (
            <td key={p.id} className={`p-3 border border-gray-100 text-center relative group transition-all duration-200
            ...
            `}>
                <div className={`font-bold text-sm ${isSubstitute ? 'text-orange-900' : isCovered ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                    {entry.subjectName}
                </div>
        ...
        <div className="p-6">
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">View your weekly classes and substitutions</p>
                </div>

                {/* Date Picker */}
                <div className="flex items-center gap-3 bg-white p-2 rounded-lg shadow-sm border border-gray-200">
                    <Calendar size={18} className="text-[#003B73]" />
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">Viewing Date:</span>
                    <input
                        type="date"
                        value={selectedDate}
                    ...
                    <table className="w-full min-w-[1000px]">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                <th className="p-4 w-24 text-left font-bold text-gray-600 text-sm uppercase tracking-wider">Day</th>
                                {periodsConsistent.map((p, idx) => (
                                    <th key={idx} className={`p-3 text-center font-bold text-gray-600 text-xs uppercase tracking-wider ${p.type === 'break' ? 'w-10 bg-gray-100' : 'w-32'}`}>
                                        {p.type === 'break' ?
                                            <span className="writing-vertical-lr transform rotate-180 h-20 text-[10px] text-gray-400">{p.name}</span> :
                                            <div>{p.label}<div className="text-[10px] text-gray-400 mt-1">{p.id}</div></div>
                                        }
                                    </th>
                                ))}
                            </tr>
